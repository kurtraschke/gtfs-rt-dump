...
import com.kurtraschke.gtfsrtdump.output.csv.FeedPart
...
@Command(name = "gtfs-rt-dump",
        ...
        subcommands = [HelpCommand::class, PbTextOutput::class, TableOutput::class, JsonOutput::class, FeedPart::class])
class Main : Callable<Int> {
    private class ProtobufSource {
        @Option(names = ["-f", "--file"],
                description = ["Read GTFS-rt from specified file."],
                required = true)
        internal var protobufPath: Path? = null

        @Option(names = ["-u", "--url"],
                description = ["Read GTFS-rt from specified URL."],
                required = true)
        internal var protobufUrl: URL? = null
    }
    ...
    @Option(names = ["-H", "--header"],
            description = ["Add specified HTTP header to request."])
    private var headers: Map<String, String> = emptyMap()

    @Option(names = ["--disable-tls-validation"],
            description = ["Disable all TLS server certificate validation."])
            ...
            val inputStream: InputStream = when {
                protobufSource?.protobufUrl != null -> {
                    ...
                    if (disableTlsValidation) {
                        ...
                        context.init(
                                null,
                                arrayOf(NullX509ExtendedTrustManager()),
                                null
                        )
                    ...
                    if (headers.isNotEmpty()) {
                        builder.headers(
                                *headers.entries
                                        .flatMap { e: Map.Entry<String, String> -> listOf(e.key, e.value) }
                                        .toTypedArray()
                        )
                    }
